...
free={(x,y) for x,y in land if -145<=x<=5 and -90<=y<=85}
for e in s['entities']:
...
free.difference_update([(-7,51),(-121,-62)])
for e in s['entities']:
...
D=[(0,-1),(1,0),(0,1),(-1,0)];start=(-7,53,2,False);end=(-121,-63)
...
while q:
 ...
 x,y,di,lock=n
 if (x,y)==end and not lock:break
 for d,(dx,dy) in enumerate(D):
  if lock and d!=di:continue
  if d==(di+2)%4:continue
  choices=[(1,1,False)]
  if not lock and d==di:choices += [(k,8 if k<=5 else 20,True) for k in range(2,8)]
...
route.reverse();json.dump(route,open('plans/lds-steel-route.json','w'),indent=2)
