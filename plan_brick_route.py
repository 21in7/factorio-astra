...
s['entities']=[e for e in s['entities'] if not(e['name']=='iron-chest' and e['position']=={'x':-12.5,'y':48.5})]
for e in s['entities']:
...
free.difference_update([(-48,-63),(-13,46)])
for e in s['entities']:
...
D=[(0,-1),(1,0),(0,1),(-1,0)];start=(-48,-62,2,False);end=(-13,48)
...
route.reverse();nodes=[tuple(route[0]['from'])]+[tuple(e['to']) for e in route];assert len(nodes)==len(set(nodes)), 'route revisits a tile';json.dump(route,open('plans/brick-route.json','w'),indent=2)
