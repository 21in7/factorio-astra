...
B('medium-electric-pole',[-125.5,-78.5])
B('medium-electric-pole',[-122.5,-73.5])
for p,r in [([-124.5,-75.5],'iron-gear-wheel'),([-120.5,-80.5],'pipe')]:B('assembling-machine-1',p);b.append({'op':'recipe','name':'assembling-machine-1','at':p,'recipe':r})
I('inserter',[-122.5,-75.5],12,'iron-gear-wheel')
I('long-handed-inserter',[-120.5,-78.5],0,'pipe')
I('inserter',[-124.5,-77.5],0,'iron-plate')
I('inserter',[-122.5,-80.5],12,'iron-plate')
I('inserter',[-96.5,-73.5],8,'iron-plate')
I('long-handed-inserter',[-119.5,-57.5],4,'steel-plate')
I('inserter',[-120.5,-73.5],8,'steel-plate')
# Fixed tail of Fe route: feeds pipe from the side then gear from above.
for p,d in [([-124.5,-80.5],8),([-124.5,-79.5],8),([-124.5,-78.5],8)]:B('transport-belt',p,d)
...
def route(start,end,ed):
 ...
 return r
r1=route((-97,-75,0,False),(-124,-81),3)
r2=route((-122,-58,3,False),(-121,-73),0)
# Endpoint inserters need power; existing poles power all except Fe source (existing acid pole covers it).
counts=Counter(a['name']for a in b if a['op']=='build');print(dict(counts));print('UG', [e for r in[r1,r2]for e in r if e['ug']])
a=[{'op':'take','name':'iron-chest','at':p,'item':'iron-plate','count':100}for p in[[34.5,39.5],[34.5,43.5],[14.5,43.5]]]
u=counts.get('underground-belt',0)//2
# Craft UG prerequisites first, long inserters before normals so subcomponents are not consumed from final builds.
recipes=[('transport-belt',math.ceil((counts['transport-belt']+u*5)/2)),('underground-belt',u),('assembling-machine-1',2),('medium-electric-pole',2),('long-handed-inserter',counts['long-handed-inserter']),('inserter',counts['inserter'])]
for r,c in recipes:
 if c:a +=[{'op':'craft','recipe':r,'count':c},{'op':'wait_crafting'}]
a+=b+[{'op':'save'}]
json.dump(a,open('plans/remote-engine-raw-auto.json','w'),indent=2)
json.dump({'iron':r1,'steel':r2},open('plans/remote-engine-raw-routes.json','w'),indent=2)
